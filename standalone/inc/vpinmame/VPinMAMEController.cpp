      ...
      if (!pController->m_serum) {
         ...
         if (pController->m_dmdUpdate) {
            if (p_displayLayout->depth == 2)
               pController->m_zedmd.RenderGray2(pController->m_pDmdData, p_displayLayout->width, p_displayLayout->height);
            else
               pController->m_zedmd.RenderGray4(pController->m_pDmdData, p_displayLayout->width, p_displayLayout->height);
         }
#endif
      }
      else if (Serum_ColorizeOrApplyRotations(pController->m_pDmdData, p_displayLayout->width, p_displayLayout->height, pController->m_dmdPalette, NULL)) {
         pController->m_dmdUpdate = true;
