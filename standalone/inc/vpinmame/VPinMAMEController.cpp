...
void VPinMAMEController::OnDisplayAvailable(int index, int displayCount, PinmameDisplayLayout *p_displayLayout)
{
   ...
   if ((p_displayLayout->type & DMD) == DMD && index == 0) {
      ...
      if (!m_serum.Load(m_szPath + "altcolor", string(m_pPinmameGame->name))) {
         ...
         generate_palette(m_dmdPalette, p_displayLayout->depth, m_dmdColor);
      }

      ZeDMDUtil::Init();
      ZeDMDUtil::Open(m_dmdWidth, m_dmdHeight);
      ZeDMDUtil::SetPalette(m_dmdPalette);
   }
}
      ...
      if (!m_serum.IsLoaded()) {
         if (p_displayData) {
            ...
            if (p_displayLayout->depth == 2)
               ZeDMDUtil::RenderGray2(m_pDmdData);
            else
               ZeDMDUtil::RenderGray4(m_pDmdData);
         }
      }
      else if (m_serum.Convert((UINT8*)p_displayData, m_pDmdData, m_dmdPalette)) {
         m_dmdUpdate = true;

         ZeDMDUtil::RenderColoredGray6(m_pDmdData, m_dmdPalette, NULL);
      }
