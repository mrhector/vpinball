      ...
      if (!m_serum.IsLoaded()) {
         if (p_displayData) {
            ...
            if (p_displayLayout->depth == 2)
               m_zedmd.RenderGray2(m_pDmdData, m_dmdWidth, m_dmdHeight);
            else
               m_zedmd.RenderGray4(m_pDmdData, m_dmdWidth, m_dmdHeight);
#endif
         }
      }
...
#if !((defined(__APPLE__) && ((defined(TARGET_OS_IOS) && TARGET_OS_IOS) || (defined(TARGET_OS_TV) && TARGET_OS_TV))))
         m_zedmd.RenderColoredGray6(m_pDmdData, m_dmdPalette, NULL, m_dmdWidth, m_dmdHeight);
#endif
      }
