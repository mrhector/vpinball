...
void CALLBACK VPinMAMEController::OnDisplayAvailable(int index, int displayCount, PinmameDisplayLayout *p_displayLayout)
{
   ...
   if ((p_displayLayout->type & DMD) == DMD && index == 0) {
      ...
      g_pController->m_dmdData = (unsigned char*) malloc(g_pController->m_dmdHeight * g_pController->m_dmdWidth);

      for (int i = 0; i <= 100; i++) {
        int r = ((float)i / 100.) * 0xFF;
        int g = ((float)i / 100.) * 0xA5;
        int b = ((float)i / 100.) * 0x00;
        g_pController->m_dmdPalette[i] = 0xFF000000 | b << 16 | g << 8 | r;
      }
   }
}
...
VPinMAMEController::VPinMAMEController()
{
   ...
   strncat((char*)config.vpmPath, g_pvp->m_szMyPath.c_str(), PINMAME_MAX_VPM_PATH - 1);
   strncat((char*)config.vpmPath, "pinmame/", PINMAME_MAX_VPM_PATH - strlen(config.vpmPath) - 1);
#else
   strncat((char*)config.vpmPath, getenv("HOME"), PINMAME_MAX_VPM_PATH - 1);
   strncat((char*)config.vpmPath, "/.pinmame/", PINMAME_MAX_VPM_PATH - strlen(config.vpmPath) - 1);
#endif
...
STDMETHODIMP VPinMAMEController::get_RawDmdColoredPixels(VARIANT *pVal)
{
   if (m_hasDMDUpdates) {
      ...
      {
         V_VT(&pData[i]) = VT_UI4;
         V_UI4(&pData[i]) = m_dmdPalette[m_dmdData[i]];
      }
      SafeArrayUnaccessData(psa);
