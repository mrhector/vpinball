...
Font* Font::Create(AssetManager* pAssetManager, AssetSrc* pAssetSrc)
{
   Font* obj = new Font();
   obj->m_pBitmapFont = (BitmapFont*)pAssetManager->Open(pAssetSrc);
 
   obj->m_textures = new SDL_Surface*[obj->m_pBitmapFont->GetPageCount()];
   memset((char*)obj->m_textures, 0, sizeof(SDL_Surface*) * obj->m_pBitmapFont->GetPageCount());

   for (int i = 0; i < obj->m_pBitmapFont->GetPageCount(); i++) {
      AssetSrc* pTextureAssetSrc = pAssetManager->ResolveSrc(obj->m_pBitmapFont->GetPage(i)->GetFilename(), pAssetSrc);
      obj->m_textures[i] = (SDL_Surface*)pAssetManager->Open(pTextureAssetSrc);
   ...
   if (pAssetSrc->GetFontBorderSize() > 0) {
      ...
      for(const auto& pair : obj->m_pBitmapFont->GetCharacters()) {
