...
void FlexDMDGroupActor::Update(float delta)
{
    ...
    while (i < m_children.size()) {
        FlexDMDBaseActor* actor = m_children[i];
        actor->Update(delta);
        if (i < m_children.size() && actor == m_children[i])
           i++;
    }
}

void FlexDMDGroupActor::Draw(SDL_Surface* pSurface)
{
    if (m_visible) {
        //graphics.TranslateTransform(m_x, m_y);
        if (m_clip) {
           SDL_Rect clipRegion = { 0, 0, (int)m_width, (int)m_height };
           SDL_SetClipRect(pSurface, &clipRegion);
...
{
    FlexDMDBaseActor* actor = (FlexDMDBaseActor*)child;
