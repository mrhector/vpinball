...
#include "scenes/FlexDMDSequence.h"
#include "actors/FlexDMDScoreBoard.h"
...
private:
   FlexDMDActor* ResolveImage(const string& filename, bool useFrame);
   Font* GetFont(const string& path, float brightness, float outlineBrightness);
   FlexDMDLabelActor* GetFittedLabel(const string& text, float fillBrightness, float outlineBrightness);
