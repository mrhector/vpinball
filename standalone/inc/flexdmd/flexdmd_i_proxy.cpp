...
#include "actors/SequenceAction.h"
#include "actors/ParallelAction.h"
#include "actors/TweenAction.h"

#include "actors/FlexDMDActionFactory.h"
...
#include "actors/FlexDMDAnimatedActor.h"
...
STDMETHODIMP FlexDMDActionFactory::GetDocumentation(INT index, BSTR *pBstrName, BSTR *pBstrDocString, DWORD *pdwHelpContext, BSTR *pBstrHelpFile) {
	if (index == MEMBERID_NIL) {
		*pBstrName = SysAllocString(L"Object");
		return S_OK;
	}
	return E_NOTIMPL;
}
...
STDMETHODIMP FlexDMDAnimatedActor::GetIDsOfNames(REFIID /*riid*/, LPOLESTR* rgszNames, UINT cNames, LCID lcid, DISPID* rgDispId) {
	static struct {
		const WCHAR *name;
		DISPID dispId;
	} namesIdsList[] = {
...
STDMETHODIMP FlexDMDAnimatedActor::Invoke(DISPID dispIdMember, REFIID /*riid*/, LCID lcid, WORD wFlags, DISPPARAMS* pDispParams, VARIANT* pVarResult, EXCEPINFO* pExcepInfo, UINT* puArgErr) {
	int index = pDispParams->cArgs;
	VARIANT res;
	HRESULT hres = DISP_E_UNKNOWNNAME;
...
STDMETHODIMP FlexDMDAnimatedActor::GetDocumentation(INT index, BSTR *pBstrName, BSTR *pBstrDocString, DWORD *pdwHelpContext, BSTR *pBstrHelpFile) {
	if (index == MEMBERID_NIL) {
		*pBstrName = SysAllocString(L"Object");
		return S_OK;
	}
