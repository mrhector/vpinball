...
void FlexDMDFrameActor::Draw(SDL_Surface* pSurface)
{
    FlexDMDBaseActor::Draw(pSurface);

    if (m_visible) {
        if (m_fill) {
            Uint32 fillColor = SDL_MapRGB(pSurface->format, GetRValue(m_fillColor), GetGValue(m_fillColor), GetBValue(m_fillColor));

            SDL_Rect rect = { (int)(m_x + m_thickness), (int)(m_y + m_thickness), (int)(m_width - 2 * m_thickness), (int)(m_height - 2 * m_thickness) };
            SDL_FillRect(pSurface, &rect, fillColor);
        }
        if (m_thickness > 0) {
              Uint32 borderColor = SDL_MapRGB(pSurface->format, GetRValue(m_borderColor), GetGValue(m_borderColor), GetBValue(m_borderColor));

              SDL_Rect rect = { (int)m_x, (int)m_y, (int)m_width, (int)m_thickness };
              SDL_FillRect(pSurface, &rect, borderColor);
