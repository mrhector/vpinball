...
STDMETHODIMP FlexDMD::NewVideo(BSTR Name, BSTR video, IVideoActor **pRetVal)
{
   ...
   if (szVideo.find("|") != std::string::npos) {
      ...
      string ext = "";
      size_t pos = szVideo.find_last_of(".");
      if (pos != string::npos) {
         ext = szVideo.substr(pos + 1);
...
STDMETHODIMP FlexDMD::NewImage(BSTR Name, BSTR image, IImageActor **pRetVal)
{
   ...
   if (szImage.find("|") != std::string::npos) {
      ...
      string ext = "";
      size_t pos = szImage.find_last_of(".");
      if (pos != string::npos) {
         ext = szImage.substr(pos + 1);
         std::transform(ext.begin(), ext.end(), ext.begin(), [](unsigned char c){ return std::tolower(c); });
      }
...
STDMETHODIMP FlexDMD::NewFont(BSTR Font, OLE_COLOR tint, OLE_COLOR borderTint, LONG borderSize, IUnknown **pRetVal)
{
   ...
   if (szFont.find("|") != std::string::npos) {
      ...
      string ext = "";
      size_t pos = szFont.find_last_of(".");
      if (pos != string::npos) {
         ext = szFont.substr(pos + 1);
         std::transform(ext.begin(), ext.end(), ext.begin(), [](unsigned char c){ return std::tolower(c); });
      }
...
#if !((defined(__APPLE__) && ((defined(TARGET_OS_IOS) && TARGET_OS_IOS) || (defined(TARGET_OS_TV) && TARGET_OS_TV))))
            m_zedmd.SetPalette(m_dmdColor, 4);
            m_zedmd.RenderGray2(dst, m_width, m_height);
#endif
         }
         break;
...
#if !((defined(__APPLE__) && ((defined(TARGET_OS_IOS) && TARGET_OS_IOS) || (defined(TARGET_OS_TV) && TARGET_OS_TV))))
            m_zedmd.SetPalette(m_dmdColor, 16);
            m_zedmd.RenderGray4(dst, m_width, m_height);
#endif
         }
         break;
...
#if !((defined(__APPLE__) && ((defined(TARGET_OS_IOS) && TARGET_OS_IOS) || (defined(TARGET_OS_TV) && TARGET_OS_TV))))
            m_zedmd.RenderRgb24((UINT8*)graphics->pixels, m_width, m_height);
#endif
         }
         break;
