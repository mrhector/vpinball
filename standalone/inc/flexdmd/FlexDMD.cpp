...
{
   FlexDMDFont* pFlexDMDLabel = (FlexDMDFont*)Font_;
   CComObject<FlexDMDLabelActor>* obj = FlexDMDLabelActor::Create(this, pFlexDMDLabel->GetFont(), MakeString(Text),  MakeString(Name));
...
{
   CComObject<FlexDMDAnimatedActor>* obj = NewVideo(MakeString(video), MakeString(Name));
...
STDMETHODIMP FlexDMD::NewImage(BSTR Name, BSTR image, IImageActor **pRetVal)
{
   ...
   CComObject<FlexDMDImageActor>* obj = FlexDMDImageActor::Create(this, m_pAssetManager, szImage, MakeString(Name));
...
{
   FlexDMDFont* obj = FlexDMDFont::Create(NewFont(MakeString(font), tint, borderTint, borderSize));
...
Font* FlexDMD::NewFont(string szFont, OLE_COLOR tint, OLE_COLOR borderTint, LONG borderSize)
{
   AssetSrc* pAssetSrc = m_pAssetManager->ResolveSrc(
        szFont + "&tint=" + color_to_hex(tint) + "&border_size=" + std::to_string(borderSize) + "&border_tint=" + color_to_hex(borderTint), NULL);
...
CComObject<FlexDMDAnimatedActor>* FlexDMD::NewVideo(string szVideo, string szName)
{
   if (szVideo.find("|") != string::npos)
      return (CComObject<FlexDMDAnimatedActor>*)FlexDMDImageSequence::Create(this, m_pAssetManager, szVideo, szName, 30, true);
   else {
      ...
      if (assetType == AssetType_Video)
         return (CComObject<FlexDMDAnimatedActor>*)FlexDMDVideoActor::Create(this, szVideo, szName, true);
      else if (assetType == AssetType_GIF)
         return (CComObject<FlexDMDAnimatedActor>*)FlexDMDGIFImage::Create(this, m_pAssetManager, szVideo, szName);
