...
STDMETHODIMP FlexDMD::NewVideo(BSTR Name, BSTR video, IVideoActor **pRetVal)
{
   ...
   if (szVideo.find("|") != std::string::npos) {
      CComObject<FlexDMDImageSequence>* obj = FlexDMDImageSequence::Create(this, szVideo, MakeString(Name), 30, true);
      ...
      if (ext == "gif") {
          CComObject<FlexDMDGIFImage>* obj = FlexDMDGIFImage::Create(this, szVideo, MakeString(Name));
...
STDMETHODIMP FlexDMD::NewImage(BSTR Name, BSTR image, IImageActor **pRetVal)
{
   ...
   if (szImage.find("|") != std::string::npos) {
      ...
      if (szImage.starts_with("VPX.") || ext == "png" || ext == "jpg" || ext == "jpeg" || ext == "bmp") {
         CComObject<FlexDMDImageActor>* obj = FlexDMDImageActor::Create(this, szImage, MakeString(Name));
...
void FlexDMD::RenderLoop()
{
   ...
   m_pGraphics = SDL_CreateRGBSurface(0, m_width, m_height, 24, 0x000000FF, 0x0000FF00, 0x00FF0000, 0);
   ...
   while (m_run) {
      ...
      if (!m_renderLockCount) {
         if (m_clear)
            SDL_FillRect(m_pGraphics, NULL, SDL_MapRGB(m_pGraphics->format, 0, 0, 0));
         ...
         switch (m_renderMode) {
            ...
            {
               UINT8* dst = bpFrame;
               UINT8* ptr = (UINT8*)m_pGraphics->pixels;
               int pos = 0;
               int x, y, r, g, b, v;
            ...
            case RenderMode_DMD_RGB:
            {
               ...
               if (m_pZeDMDUtil)
                  m_pZeDMDUtil->RenderRgb24((UINT8*)m_pGraphics->pixels);
            }
            break;
   ...
   SDL_FreeSurface(m_pGraphics);

   m_pGraphics = NULL;
}

SDL_Surface* FlexDMD::GetGraphics()
{
   return m_pGraphics;
}