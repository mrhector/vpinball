...
{
    CComObject<FlexDMDLabelActor>* obj = FlexDMDLabelActor::Create(this, MakeString(Name), (void*)Font, MakeString(Text));
...
STDMETHODIMP FlexDMD::NewVideo(BSTR Name, BSTR video, IVideoActor **pRetVal)
{
    ...
    if (szVideo.find("|") != std::string::npos) {
       ...
       if (ext == ".gif") {
           CComObject<FlexDMDGIFImage>* obj = FlexDMDGIFImage::Create(this, szName, szVideo);
...
{
    CComObject<FlexDMDImageActor>* obj = FlexDMDImageActor::Create(this, MakeString(Name), MakeString(image));
