...
SDL_FRect GraphicsPath::GetBounds(B2S::Matrix* pMatrix) const
{
...
void GraphicsPath::Transform(B2S::Matrix* pMatrix)
{
   if (pMatrix)
...
int GraphicsPath::FlattenPath(GraphicsPath* pPath, B2S::Matrix* pMatrix, float flatness)
{
   int status = 0;
