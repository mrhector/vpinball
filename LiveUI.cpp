...
#endif
#ifdef __STANDALONE__
      if (m_live_table->m_BG_current_set != 0) {
         if (ImGui::Button("Desktop", size)) {
            m_useEditorCam = false;
            m_live_table->m_BG_current_set = 0;
            ImGui::CloseCurrentPopup();
            HideUI();
         }
      }
      if (m_live_table->m_BG_current_set != 1) {
         if (ImGui::Button("Cabinet", size)) {
            m_useEditorCam = false;
            m_live_table->m_BG_current_set = 1;
            ImGui::CloseCurrentPopup();
            HideUI();
         }
      }
      if (m_live_table->m_BG_current_set != 2) {
         if (ImGui::Button("Full Single Screen", size)) {
            m_useEditorCam = false;
            m_live_table->m_BG_current_set = 2;
            ImGui::CloseCurrentPopup();
            HideUI();
         }
      }
