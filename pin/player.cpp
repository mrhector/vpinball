...
#ifdef __STANDALONE__
   m_pB2SBackglassWindow = nullptr;
   m_pB2SBackglassRenderer = nullptr;
   m_b2sBackglassRenderCallback = nullptr;

   m_pB2SDMDWindow = nullptr;
   m_pB2SDMDRenderer = nullptr;
   m_b2sDMDRenderCallback = nullptr;
...
void Player::CreateWnd(HWND parent /* = 0 */)
{
...
#ifdef __STANDALONE__
   flags = SDL_WINDOW_SKIP_TASKBAR | SDL_WINDOW_BORDERLESS | SDL_WINDOW_HIDDEN | SDL_WINDOW_UTILITY;
   ...
   if (b2s) {
      m_pB2SBackglassWindow = SDL_CreateWindow("vpx_b2sbackglass",
         ...
         LoadValueWithDefault(regKey[RegName::Standalone], "B2SBackglassHeight"s, 0), flags);

      if (m_pB2SBackglassWindow) {
         int x, y, w, h;
         SDL_GetWindowPosition(m_pB2SBackglassWindow, &x, &y);
         SDL_GetWindowSize(m_pB2SBackglassWindow, &w, &h);
         PLOGI.printf("B2S Backglass window created: %d, %d (%dx%d)", x, y, w, h);

         m_pB2SBackglassRenderer = SDL_CreateRenderer(m_pB2SBackglassWindow, -1, SDL_RENDERER_ACCELERATED);
      }

      m_pB2SDMDWindow = SDL_CreateWindow("vpx_b2sdmd",
         ...
         LoadValueWithDefault(regKey[RegName::Standalone], "B2SDMDHeight"s, 0), flags);

      if (m_pB2SDMDWindow) {
         int x, y, w, h;
         SDL_GetWindowPosition(m_pB2SDMDWindow, &x, &y);
         SDL_GetWindowSize(m_pB2SDMDWindow, &w, &h);
         PLOGI.printf("B2S DMD window created: %d, %d (%dx%d)", x, y, w, h);

         m_pB2SDMDRenderer = SDL_CreateRenderer(m_pB2SDMDWindow, -1, SDL_RENDERER_ACCELERATED);
      }
   }
   ...
   if (dmd) {
      m_pDMDWindow = SDL_CreateWindow("vpx_dmd",
         ...
         LoadValueWithDefault(regKey[RegName::Standalone], "DMDHeight"s, 0),  flags);

      if (m_pDMDWindow) {
         int x, y, w, h;
         SDL_GetWindowPosition(m_pDMDWindow, &x, &y);
         SDL_GetWindowSize(m_pDMDWindow, &w, &h);
         PLOGI.printf("DMD window created: %d, %d (%dx%d)", x, y, w, h);
...
#ifdef __STANDALONE__
void Player::SetB2SBackglassRenderCallback(RenderCallback callback)
{
   if (!m_pB2SBackglassWindow || !m_pB2SBackglassRenderer)
      return;

   m_b2sBackglassRenderCallback = callback;
   if (callback)
      SDL_ShowWindow(m_pB2SBackglassWindow);
   else
      SDL_HideWindow(m_pB2SBackglassWindow);

   SDL_RaiseWindow(m_sdl_playfieldHwnd);
}

void Player::SetB2SDMDRenderCallback(RenderCallback callback)
{
   if (!m_pB2SDMDWindow || !m_pB2SDMDRenderer)
      return;

   m_b2sDMDRenderCallback = callback;
   if (callback)
      SDL_ShowWindow(m_pB2SDMDWindow);
   else
      SDL_HideWindow(m_pB2SDMDWindow);

   SDL_RaiseWindow(m_sdl_playfieldHwnd);
}

void Player::SetDMDRenderCallback(RenderCallback callback)
{
   if (!m_pDMDWindow || !m_pDMDRenderer)
      return;

   m_dmdRenderCallback = callback;
   if (callback)
      SDL_ShowWindow(m_pDMDWindow);
   else
      SDL_HideWindow(m_pDMDWindow);
...
void Player::Shutdown()
{
   ...
   if (m_pB2SBackglassRenderer)
      SDL_DestroyRenderer(m_pB2SBackglassRenderer);

   if (m_pB2SBackglassWindow)
      SDL_DestroyWindow(m_pB2SBackglassWindow);

   m_b2sDMDRenderCallback = nullptr;

   if (m_pB2SDMDRenderer)
      SDL_DestroyRenderer(m_pB2SDMDRenderer);

   if (m_pB2SDMDWindow)
      SDL_DestroyWindow(m_pB2SDMDWindow);
...
#ifdef __STANDALONE__
   if (g_pplayer->m_b2sBackglassRenderCallback)
      g_pplayer->m_b2sBackglassRenderCallback(g_pplayer->m_pB2SBackglassRenderer);

   if (g_pplayer->m_b2sDMDRenderCallback)
      g_pplayer->m_b2sDMDRenderCallback(g_pplayer->m_pB2SDMDRenderer);

   if (g_pplayer->m_dmdRenderCallback)
      g_pplayer->m_dmdRenderCallback(g_pplayer->m_pDMDRenderer);

   if (g_pplayer->m_b2sBackglassRenderCallback || m_b2sDMDRenderCallback || g_pplayer->m_dmdRenderCallback)
