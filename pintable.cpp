...
STDMETHODIMP ScriptGlobalTable::PlayMusic(BSTR str, float volume)
{
   if (g_pplayer && g_pplayer->m_PlayMusic)
   {
      ...
      for (int i = 0; i < strnlen(szT, sizeof(szT)); i++) {
         if (szT[i] == '\\' || szT[i] == '/')
            szT[i] = PATH_SEPARATOR_CHAR;
      }
#endif
...
void PinTable::Play(const bool cameraMode)
{
   ...
   Timer* implicitTimer = (Timer*)EditableRegistry::CreateAndInit(ItemTypeEnum::eItemTimer, live_table, 0, 0);
   live_table->m_pcv->RemoveItem(implicitTimer->GetScriptable());
   wcscpy(implicitTimer->m_wzName, L"ImplicitTimer");
   implicitTimer->m_d.m_tdr.m_TimerEnabled = true;
   implicitTimer->m_d.m_tdr.m_TimerInterval = 100;
   live_table->m_vedit.push_back(implicitTimer);
   live_table->m_pcv->AddItem(implicitTimer->GetScriptable(), false);
   szT = MakeChar(implicitTimer->m_wzName);
   PLOGI << "Implicit Timer added: name=" << szT;
   delete[] szT;
#endif
