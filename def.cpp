...
bool try_parse_color(const std::string& str, OLE_COLOR& value)
{
   string hexStr = str;
...
int string_to_int(const std::string& str, int defaultValue)
{
   int value;
...
HRESULT external_create_object(const WCHAR* progid, IClassFactory* cf, IUnknown* obj)
{
   ...
   if (!wcsicmp(progid, L"VPinMAME.Controller")) {
      ...
      CComObject<UltraDMDDMDObject>* pObj = nullptr;
      if (SUCCEEDED(CComObject<UltraDMDDMDObject>::CreateInstance(&pObj))) {
         hres = pObj->QueryInterface(IID_IDMDObject, (void**)obj);
      }
   }
