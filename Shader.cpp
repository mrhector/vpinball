...
Shader::ShaderTechnique* Shader::compileGLShader(const ShaderTechniques technique, const string& fileNameRoot, const string& shaderCodeName, const string& vertex, const string& geometry, const string& fragment)
{
   ...
   if (result == FALSE)
   {
      ...
      PLOGE << shaderCodeName << ": Vertex Shader compilation failed with:" << std::endl << errorText;
      char msg[2048];
...
#ifdef __STANDALONE__
      PLOGE << "vertex:" << std::endl << add_line_numbers(vertexSource);
#endif
   }
#ifndef __STANDALONE__
   //Geometry Shader
...
#ifdef __STANDALONE__
         PLOGE << "fragment:" << std::endl << add_line_numbers(fragmentSource);
#endif
      }
   }
   ...
   if (success) {
      ...
      if (result == GL_FALSE)
      {
         ...
         PLOGE << shaderCodeName << ": Linking Shader failed with: " << std::endl << errorText;
         free(errorText);
         success = false;
